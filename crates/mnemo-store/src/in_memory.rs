        ...
        if let Some(conflict_key) = memory.conflict_key() {
            ...
            for existing in memories.values_mut() {
                if existing.namespace().stable_key() == namespace_key
                    && existing.conflict_key() == Some(conflict_key.as_str())
                    && existing.status() == MemoryStatus::Active
                    && existing.memory_id() != memory.memory_id()
                {
                    existing.mark_superseded_by(memory.memory_id());
                    memory.add_supersedes(existing.memory_id());
                    superseded_memory_ids.push(existing.memory_id().to_string());
                }
            }
        ...
        let mut active = memories
            ...
            .filter(|memory| {
                memory.namespace().stable_key() == namespace_key
                    && memory.status().is_context_pack_eligible()
            })
            .cloned()
            .collect::<Vec<_>>();
